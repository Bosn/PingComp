...
app.get('/logout', (req, res) => {
    if (!authEnabled || !res.oidc?.logout)
        return res.redirect('/');
...
async function logActivity(conn, leadId, action, actor = 'pingcomp-ui', beforeObj = null, afterObj = null) {
    try {
        await conn.execute(`INSERT INTO lead_activity_log (lead_id, action, actor, before_json, after_json) VALUES (?, ?, ?, ?, ?)`, [leadId, action, actor, beforeObj ? JSON.stringify(beforeObj) : null, afterObj ? JSON.stringify(afterObj) : null]);
    }
    catch { }
}
...
async function findLeadIdByName(conn, name, excludeId) {
...
app.post('/api/interviews', async (req, res) => {
    ...
    if (!interviewDate)
        return res.status(400).json({ ok: false, error: 'missing interviewDate' });
    if (!channel)
        return res.status(400).json({ ok: false, error: 'missing channel' });
    if (!transcriptHtml)
        return res.status(400).json({ ok: false, error: 'missing transcriptHtml' });
    const transcriptPlain = htmlToPlain(transcriptHtml);
    const actor = getActor(req);
    const conn = await getConn();
    try {
        const [r] = await conn.execute(`INSERT INTO interviews (
        ...
        const [[row]] = await conn.query(`SELECT * FROM interviews WHERE id=? AND deleted_at IS NULL`, [id]);
...
app.put('/api/interviews/:id', async (req, res) => {
    ...
    const conn = await getConn();
    try {
        const [[before]] = await conn.query(`SELECT * FROM interviews WHERE id=? AND deleted_at IS NULL`, [id]);
        if (!before)
            return res.status(404).json({ ok: false, error: 'not found' });
        ...
        await conn.execute(`UPDATE interviews SET
...
app.delete('/api/interviews/:id', async (req, res) => {
    ...
    const conn = await getConn();
    try {
        const [[row]] = await conn.query(`SELECT id FROM interviews WHERE id=? AND deleted_at IS NULL`, [id]);
        if (!row)
            return res.status(404).json({ ok: false, error: 'not found' });
...
function renderInterviewMarkdown(interview, leadName) {
    const tags = String(interview?.tags || '').split(',').map((s) => s.trim()).filter(Boolean);
    const lines = [];
...
function ensureAuthForNonApi(req, res, next) {
...
app.get('/api/interviews', async (req, res) => {
    ...
    try {
        ...
        if (interviewer) {
            where += ' AND interviewer LIKE ?';
            args.push(`%${interviewer}%`);
        }
        if (from) {
            where += ' AND interview_date >= ?';
            args.push(from);
        }
        if (to) {
            where += ' AND interview_date <= ?';
        ...
        const [rows] = await conn.query(`SELECT id,lead_id,title,interview_date,channel,interviewer,company,summary,tags,updated_at
...
app.get('/interviews/:id/export.md', ensureAuthForNonApi, async (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isFinite(id) || id <= 0)
        return res.status(400).send('invalid id');
    const conn = await getConn();
    try {
        const [[it]] = await conn.query(`SELECT * FROM interviews WHERE id=? AND deleted_at IS NULL`, [id]);
        if (!it)
            return res.status(404).send('Not found');
        const [[lead]] = await conn.query(`SELECT id,name FROM \`${TABLE}\` WHERE id=?`, [Number(it.lead_id)]);
...
app.get('/interviews/export.md', ensureAuthForNonApi, async (req, res) => {
    const leadId = String(req.query.leadId || '').trim();
    const q = String(req.query.q || '').trim();
    const channel = String(req.query.channel || '').trim();
    const interviewer = String(req.query.interviewer || '').trim();
    ...
    try {
        ...
        const [countRows] = await conn.query(`SELECT COUNT(*) c FROM interviews ${where}`, args);
        ...
        const [rows] = await conn.query(`SELECT * FROM interviews ${where} ORDER BY interview_date DESC, id DESC`, args);
        // Build leadId->name map to avoid per-row queries.
        const leadIds = Array.from(new Set((rows || []).map((r) => Number(r.lead_id)).filter((n) => Number.isFinite(n))));
        let leadMap = new Map();
        if (leadIds.length) {
            const placeholders = leadIds.map(() => '?').join(',');
            const [leadRows] = await conn.query(`SELECT id,name FROM \`${TABLE}\` WHERE id IN (${placeholders})`, leadIds);
            for (const lr of (leadRows || []))
                leadMap.set(Number(lr.id), String(lr.name || ''));
        }
        const headerLines = [];
...
app.post('/api/leads', async (req, res) => {
    ...
    try {
        ...
        if (existingId)
            return res.status(409).json(leadNameConflictPayload(name));
        let ret;
        try {
            [ret] = await conn.execute(`INSERT INTO \`${TABLE}\`
...
app.get('/api/leads', async (req, res) => {
    ...
    const [countRows] = await conn.query(`SELECT COUNT(*) c FROM \`${TABLE}\` ${where}`, args);
    const total = Number((countRows?.[0]?.c) || 0);
...
app.get('/api/leads/:id', async (req, res) => {
    const conn = await getConn();
    const [rows] = await conn.query(`SELECT * FROM \`${TABLE}\` WHERE id=?`, [req.params.id]);
    await conn.end();
    if (!rows?.[0])
...
app.put('/api/leads/:id', async (req, res) => {
    ...
    const conn = await getConn();
    try {
        const [beforeRows] = await conn.query(`SELECT * FROM \`${TABLE}\` WHERE id=?`, [id]);
...
app.delete('/api/leads/:id', async (req, res) => {
    ...
    if (!Number.isFinite(id) || id <= 0)
        return res.status(400).json({ ok: false, error: 'invalid id' });
    const conn = await getConn();
    const [[row]] = await conn.query(`SELECT * FROM \`${TABLE}\` WHERE id=?`, [id]);
    if (!row) {
        await conn.end();
        return res.status(404).json({ ok: false, error: 'not found' });
    }
...
app.post('/api/agent/chat', async (req, res) => {
    ...
    `);
        ...
        for (const r of (bucketRows || []))
            map.set(Number(r.b || 0), Number(r.c || 0));
        const labels = [];
        const values = [];
    ...
    const runQwen = async (system, user) => {
        if (!qwenKey)
            throw new Error('missing_qwen_api_key');
        ...
        return String(jj?.choices?.[0]?.message?.content ||
            jj?.output?.choices?.[0]?.message?.content ||
            jj?.output?.text || '').trim();
    };
    const isSafeReadOnlySql = (sqlRaw) => {
        const sql = sqlRaw.trim().replace(/;\s*$/, '');
        if (!/^select\b/i.test(sql))
            return false;
        if (/\b(insert|update|delete|replace|drop|alter|truncate|create|grant|revoke|call|execute|show|desc|describe|explain|set|use|commit|rollback)\b/i.test(sql))
            return false;
        if (/--|\/\*|\*\//.test(sql))
            return false;
        if (/\b(information_schema|mysql\.|performance_schema|sys\.)\b/i.test(sql))
            return false;
    ...
    if (qwenKey) {
        try {
            ...
            if (sql && isSafeReadOnlySql(sql)) {
                ...
                const [qRows] = await conn.query(sqlUsed);
       ...
       LIMIT 200`, fallbackArgs);
        ...
        if (qwenKey) {
            try {
                ...
                const ids = Array.isArray(parsed2?.ids) ? parsed2.ids.map((x) => Number(x)).filter((x) => Number.isFinite(x)) : [];
                const idSet = new Set(ids);
                rows = pool.filter((r) => idSet.has(Number(r.id))).slice(0, 50);
                if (!rows.length)
    ...
    const names = rows.slice(0, 5).map((r) => `${r.name}(${r.tidb_potential_score ?? '-'})`).join('，');
...
app.get('/api/regions', async (req, res) => {
    ...
    const [rows] = await conn.query(`SELECT region, COUNT(*) c FROM \`${TABLE}\` ${where} GROUP BY region ORDER BY c DESC, region ASC LIMIT 100`, args);
    await conn.end();
    res.json({
...
app.get('/api/dashboard', async (_req, res) => {
  ...
  `);
    ...
    ORDER BY c DESC
  `);
    const dailyTrend = Array.isArray(trendRows) ? trendRows.map((r) => ({ d: String(r.d), c: Number(r.c || 0) })) : [];
    await conn.end();
    res.json({ total: tot?.c || 0, locked: locked?.c || 0, avgScore: avgScore?.score || 0, statusRows, topRows, dailyTrend, scoreBuckets, enrichRows });
});
app.get('/api/enrich/queue', async (_req, res) => {
    const conn = await getConn();
...
app.post('/api/enrich/enqueue', async (req, res) => {
    ...
    const conn = await getConn();
    for (const id of ids) {
        await conn.execute(`INSERT INTO lead_enrichment_queue (lead_id, status, attempts)
       VALUES (?, 'pending', 0)
       ON DUPLICATE KEY UPDATE updated_at=CURRENT_TIMESTAMP`, [id]);
        await logActivity(conn, id, 'enrich_enqueue');
    }
    await conn.end();
    res.json({ ok: true, queued: ids.length });
});
app.post('/api/enrich/run', async (_req, res) => {
    const conn = await getConn();
...
app.get('/app/*', ensureAuth, (_req, res) => res.sendFile(process.cwd() + '/web/dist/index.html'));
...
app.get('/dashboard', (_req, res) => res.redirect('/app'));
app.get('/enrich', (_req, res) => res.redirect('/app'));
app.get('/activity', (_req, res) => res.redirect('/app'));
app.get('/export', (_req, res) => res.redirect('/app'));
